...
7. **区域覆盖多元化**：不再局限伊朗，执行摘要与时间线至少覆盖 3 个不同区域（中东 / 俄乌 / 东亚 / 欧美政治 / 突发事件）中存在实质新闻的。若某区域过去 8 小时无新闻，在执行摘要中显式说明 "无重大更新"。
...
SINCE=$(date -u -d '8 hours ago' +"%Y-%m-%d %H:%M UTC")
REPORT=report/${TS}.md
```
...
1. `WebSearch`：`CNN` + 过去 8 小时 + 关键词（Middle East, Russia Ukraine, China Taiwan, oil, markets, breaking）
2. `WebSearch`：`BBC` + 过去 8 小时 + 关键词（same set）
...
| 序号 | 占位符 | 要求 |
|---|---|---|
| 3.1 | `SECTION_1_EXEC_SUMMARY` | 3-6 条加粗关键词 + 一句话，覆盖地缘 / 能源 / 市场 |
...
| 3.17 | `SECTION_7_HOTSPOTS` | Markdown 表格；每行一个区域议题；仅保留过去 8h 有进展的 |
...
- 若过去 8 小时信息极少（例如周末），在对应小节写 `过去 8 小时无实质性更新`，不要凑字。
